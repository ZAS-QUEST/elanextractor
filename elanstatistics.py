...
def getVernacularWords(root, timeslots, alignableannotations, filename):
    ...
    for candidate in transcriptioncandidates:
        ...
        if vernaculartiers != []:  # we found a tier of the linguistic type
            ...
            for t in vernaculartiers:
                ...
                timelistannno = [
                    getDuration(alignableannotations.get(ra.attrib["ANNOTATION_REF"]))
    ...
    if not tierfound:  # there is no tier of the relevant type
        print(
            filename,
            list(
                set([x.attrib["LINGUISTIC_TYPE_REF"] for x in root.findall(".//TIER")])
            ),
        )
        ...
        if translationtiers != []:  # we found a tier of the linguistic type
            for t in translationtiers:
                ...
                if toplanguage.prob < langdetectthreshold:
                    ...
                    continue
                translation_minimum = 1.5 #how many words should the average annotation have for this tier to be counted as translation?
                avg_annotation_length = sum([len(x.strip().split()) for x in wordlist])/len(wordlist)
                if avg_annotation_length < translation_minimum: 
                    logging.warning(
                        '%s has too short annotations (%s) for the tier to be a translation (%s ,...)' %(tierID, avg_annotation_length, ", ".join(wordlist[:3]))
                    )
                    continue                       
                try:
...
if __name__ == "__main__":
    ...
    if os.path.isfile(filename):  # argument is a single file FIXME currently broken
        ...
        seconds, words = getTranscription(
        ...
        for eaf in eafs:
            ...
            alignableannotations = getAlignableAnnotations(root)
            transcriptionresults = getVernacularWords(
                root, timeslots, alignableannotations, eaf
            )
        ...
        englishwordcount = sum(
            [len(tier) for key in eaftranslations for tier in eaftranslations[key]]
        )
        ...
        print(
            "Total translations into English have %i words in %i files of directory %s/ (of total %i files)"
            % (
                englishwordcount,
